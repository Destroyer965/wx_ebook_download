<template>
	<view :style="{
		...
		<!-- 占位图使用背景图的方式 -->
		<view :class="['content',over?'over':'',useCssLoading?'animation':'']" :style="{
			borderRadius,
			background:useCssLoading?'':`url(${loadSrc})`
		}">
			<!-- 加载成功 -->
			<image class="loadOver" @load="load" @error="error" v-if="status==1" :src="src" mode="scaleToFill" :style="{
					opacity:isShow?'1':'0',
					borderRadius,
			...
			<!-- 加载失败 -->
			<image class="loadOver" v-if="status==2" :src="errorSrc" mode="scaleToFill" :style="{
				opacity:isShow?'1':'0',
				borderRadius,
				transition: `opacity ${duration/1000}s ${effect}`
				}">
...
<script>
	...
	export default {
		...
		mounted() {
			...
			intersectionObserver.relativeToViewport(this.showDistance).observe('.content', (res) => {
